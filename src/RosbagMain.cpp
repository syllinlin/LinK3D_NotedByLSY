...
{ 
    ...
    nh.param<int>("scan_line", nScans, 32); // 默认参数 = 32
    ...
    T.pretranslate(Eigen::Vector3d(0, 0, 30)); // pretranslate表示左乘 以世界系为参考系沿着Z轴方向移动30
    ...
    while (ros::ok())
    {
        if(!cloudBuffer.empty())
        {
            ...
            // Step 1 获得当前点云 存储在变量currentCloud中
            mBuf.lock();
            ...
            // Step 2 提取keypoints 并生成相应的描述子
            vector<pcl::PointXYZ> currentKeyPoints;
            cv::Mat currentDescriptors;
            vector<int> currentIndex;
            MatPt currentClustered;
            MatPt curHighSmooth;
            // operator函数加速 keypoints的提取、以及对应描述子的生成 —— 会完成依次完成边缘点提取、点云聚类、聚类关键点提取、描述子生成等
            ...
            if(count > 0) 
            {
                ...
                // Step 5 根据经过Step 3后得到的true edge keypoints完成帧间匹配 —— matchedEdgePoint
